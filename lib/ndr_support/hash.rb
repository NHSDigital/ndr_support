class Hash
  ...
  def rawtext_merge(hash2, prevent_overwrite = true)
    ...
    if prevent_overwrite
      ...
      raise(RuntimeError, "Non-unique rawtext keys: #{non_unique_rawtext_keys.inspect}") unless non_unique_rawtext_keys.empty?
      non_unique_non_rawtext_keys = (self.keys & hash2.keys) - [:rawtext]
      raise(RuntimeError, "Non-unique non-rawtext keys: #{non_unique_non_rawtext_keys.inspect}") unless non_unique_non_rawtext_keys.empty?
    end
    
    return self.merge(hash2).merge({
