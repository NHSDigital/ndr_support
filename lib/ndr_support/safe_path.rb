...
# permissions. The path space is a directory. Everything in this directory and all the subdirectories
# can be accessed with the permissions given to the constructor. The instance of the class checks whether the 
# path constructed points to a directory, whcih is in the "path space". The idea is to limit the access of the 
# program to given directory.
#
# Example of usage is :
...
class SafePath  
  ...
  def initialize(path_space, root_suffix = "")
    ...
    raise ArgumentError, "The space #{path_space} doesn't exist. Please choose one of: #{fs_paths[platform].keys.inspect}" unless fs_paths[platform][path_space]
    raise ArgumentError, "The space #{path_space} is broken. The root path should be absolute path but it was #{fs_paths[platform][path_space]["root"]}" unless fs_paths[platform][path_space]["root"].match(root_path_regexp)
    raise ArgumentError, "The space #{path_space} is broken. No permissions specified}" unless fs_paths[platform][path_space]["prms"]
  ...
  def permissions=(permissions)
    ...
    @prm = [permissions].flatten.each do |prm|
        raise ArgumentError, err_mess unless @maximum_prms.include?(prm)
    end.uniq
  end
  ...
  def verify(path)
    ...
    raise SecurityError, "The given path is insecure. The path should point at #{@root}, but it points at #{epath}" unless epath.match(/\A#{Regexp.quote(@root)}/)
    epath
  end
