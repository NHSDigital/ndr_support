class SafeFile  
  ...
  def self.readlines(*args)
    raise ArgumentError, "Incorrect number of arguments - #{args.length}" if args.length > 2 or args.length == 0    
    verify args[0], "r"
    File.readlines(*args)
  end
  ...
  def self.verify(file_names, prm = nil)
    [file_names].flatten.each do |file_name|
      raise ArgumentError, "file_name should be of type SafePath, but it is #{file_name.class}" unless file_name.class == SafePath
      
      if prm
        [prm].flatten.each do |p|
          raise SecurityError, "Permissions denied. Cannot access the file #{file_name} with permissions #{prm}. The permissions are #{file_name.permissions}" unless file_name.permissions.include?(p)
        end
      end
  ...
  def self.verify_mode(file_name, mode)
    if mode.match(/\A(r\+)|(w\+)|(a\+)\Z/)
      ...
      raise ArgumentError, "Incorrect mode. It should be one of: 'r', 'w', 'r+', 'w+', 'a', 'a+'"
    end
  end
  ...
  def self.get_fname_mode_prms(*args)
    ...
    when 2
      raise ArgimentError if args[1].class != Fixnum and args[1].class != String
    ...
    when 3
      raise ArgimentError if args[1].class != String or args[2].class != Fixnum
      verify_mode(args[0], args[1])
      ...
      raise ArgumentError, "Incorrect number of arguments #{args.length}"
    end
