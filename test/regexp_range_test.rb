...
YAML
  ...
  test 'to_range with number and number' do
    ...
    assert_equal @lines[Range.new(2, 3, true)], @lines[RegexpRange.new(2, 3, true).to_range(@lines)]
    assert_equal @lines[Range.new(2, 3, false)], @lines[RegexpRange.new(2, 3, false).to_range(@lines)]
  end
  ...
  test 'to_range with number and regexp' do
    ...
    assert_equal @lines[Range.new(2, 3, true)], @lines[RegexpRange.new(2, /^3Ut/, true).to_range(@lines)]
    assert_equal @lines[Range.new(2, 3, false)], @lines[RegexpRange.new(2, /^3Ut/, false).to_range(@lines)]
  ...
  test 'to_range with regexp and number' do
    ...
    assert_equal @lines[Range.new(1, -1, true)], @lines[RegexpRange.new(/^1consec/, -1, true).to_range(@lines)]
    assert_equal @lines[Range.new(1, -1, false)], @lines[RegexpRange.new(/^1consec/, -1, false).to_range(@lines)]
  ...
  test 'to_range with regexp and regexp' do
    assert_equal Range.new(1, 3, true), RegexpRange.new(/^1consec/, /^3Ut/, true).to_range(@lines)
    assert_equal Range.new(1, 3, false), RegexpRange.new(/^1consec/, /^3Ut/, false).to_range(@lines)

    assert_equal @lines[Range.new(1, 3, true)], @lines[RegexpRange.new(/^1consec/, /^3Ut/, true).to_range(@lines)]
    assert_equal @lines[Range.new(1, 3, false)], @lines[RegexpRange.new(/^1consec/, /^3Ut/, false).to_range(@lines)]
